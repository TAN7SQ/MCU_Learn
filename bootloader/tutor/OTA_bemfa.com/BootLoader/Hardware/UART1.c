...
char Serial_RxPacket[512]; // 定义接收数据包数组
...
{
	/*开启时钟*/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE); // 开启USART1的时钟,它属于APB2
	...
	USART_ITConfig(USART1, USART_IT_IDLE, ENABLE); // 开启串口接收数据空闲标志
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 4; // 指定NVIC线路的抢占优先级
...
void USART1_IRQHandler(void)
{
	...
	else if (USART_GetITStatus(USART1, USART_IT_IDLE) == SET) // 触发中断,同时USART1的空闲位置1,表示数据传输完毕了
	{
