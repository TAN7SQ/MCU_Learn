...
/* private function --------------------------------------------------------- */
    ...
    static void _get_init_export_table(void)
    {
        ...
        while (1)
        {
            address_last = ((elab_pointer_t)func_block - sizeof(elab_export_t)); // 向前遍历，直到对比的head和tail值不一样
        ...
        export_init_table = func_block; // 这样就得到了地址最低的export_init_t结构体
        ...
        while (1)
        {
                ...
                export_init_table[i].magic_tail == EXPORT_ID_INIT)
            {
                ...
                i++; // 只需要地址自增就能实现遍历，找到尾，这样就直到有多少个函数了
            }
            else
            {
                break;
